...
DB_PATH = "bateria_vdc.db"

# Reads the dataset information
def llegir_dades():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT timestamp, value FROM vdc_data ORDER BY timestamp ASC")
...
def main():
    ...
    # Creates and shows the graphic
    plt.figure(figsize=(12,6))
    plt.plot(timestamps, valors, marker='o', linestyle='-', color='purple', label='Vdc (V)')
    plt.title('Evolució de la tensió Vdc')
    plt.xlabel('Temps')
    plt.ylabel('Tensió (V)')
    plt.grid(True)
    plt.legend()
