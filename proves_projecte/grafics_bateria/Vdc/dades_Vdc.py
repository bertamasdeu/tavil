...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS vdc_data (
...
def insert_vdc_data(timestamp, value):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO vdc_data (timestamp, value) VALUES (?, ?)", (timestamp, value))
    conn.commit()
    conn.close()
    ...
    async with Client(url=SERVER_URL) as client:
        ...
        while True:
            try:
                ...
                print(f"[{timestamp}] Vdc: {value}")

                # Saves the data in the dataset
                insert_vdc_data(timestamp, value)
