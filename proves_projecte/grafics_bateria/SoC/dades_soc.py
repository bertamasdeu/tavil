...
def insert_soc_data(timestamp, value):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO soc_data (timestamp, value) VALUES (?, ?)", (timestamp, value))
    conn.commit()
    conn.close()
    ...
    async with Client(url=SERVER_URL) as client:
        ...
        while True:
            try:
                ...
                print(f"[{timestamp}] SoC: {value}")

                # Saves the data in the dataset
                insert_soc_data(timestamp, value)
