...
NODE_ID = "ns=6;s=::AsGlobalPV:Bateria.SoH"
...
def insert_soh_data(timestamp, value):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO soh_data (timestamp, value) VALUES (?, ?)", (timestamp, value))
    conn.commit()
    conn.close()
    ...
    async with Client(url=SERVER_URL) as client:
        ...
        while True:
            try:
                ...
                print(f"[{timestamp}] SoH: {value}")

                # Saves the data in the dataset
                insert_soh_data(timestamp, value)
